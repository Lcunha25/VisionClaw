...
struct HomeView: View {
    ...
    VStack(alignment: .leading, spacing: 0) {
      ...
      if let warning = viewModel.operationsSyncWarning,
        ...
        statusBanner(
          title: "SYNC WARNING",
          body: warning,
          accent: DesignSystem.colors.warningAmber
        )
        .padding(.bottom, viewModel.pendingTaskSOPs.isEmpty ? 0 : 10)
      }
  ...
  private func noticeBanner(title: String, body: String) -> some View {
    statusBanner(
      title: title,
      body: body,
      accent: DesignSystem.colors.vibrantTeal
    )
  }

  private func statusBanner(title: String, body: String, accent: Color) -> some View {
    ...
    .frame(maxWidth: .infinity, alignment: .leading)
    .background(DesignSystem.colors.deepNavy.opacity(0.75))
    .overlay(Rectangle().stroke(accent.opacity(0.32), lineWidth: 1))
  }
